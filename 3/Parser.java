  ...
  boolean match(String expTok){
    ...
    if(parseError == 0){
      if(stream.get(i).type.equals(expTok)){
        if(expTok.equals("Id")){
          tree.addLeaf(stream.get(i).name, "Id", stream.get(i).lineNum, stream.get(i).indexNum);
        }
        else if(expTok.equals("Digit")){
          tree.addBranch(expTok);
          tree.addLeaf(Integer.toString(stream.get(i).value), "Digit", stream.get(i).lineNum, stream.get(i).indexNum);
        }
        else if(expTok.equals("char")){
          tree.addLeaf(stream.get(i).name, "Char", stream.get(i).lineNum, stream.get(i).indexNum);
        }
        else{
          tree.addLeaf(expTok, expTok, stream.get(i).lineNum, stream.get(i).indexNum);
        }
        retVal = true;
      }
